// Bulk song-metadata lookup over the iTunes Search API (PRD-remote-falcon-003).
//
// Sequentially looks up a list of sequences and proposes the FIRST match for
// each; the caller shows the proposals in a review table before anything is
// committed. Pacing is contract: Apple documents the Search API at
// "approximately 20 calls per minute", so items run one per interval
// (default ~17/min) rather than in a burst. A 100-sequence pass takes about
// six minutes, so the UI must show progress and offer cancel.

import { lookupITunes } from './musicMetadata';

export const BULK_LOOKUP_INTERVAL_MS = 3500;

// Rounded whole-minute estimate for a full pass, floored at 1. Shared by
// the confirm interstitial and the running dialog so they never disagree.
export const estimateBulkLookupMinutes = (count) => Math.max(1, Math.round((count * BULK_LOOKUP_INTERVAL_MS) / 60000));

// Abort-aware sleep: resolves early (never rejects) when the signal fires.
const sleep = (ms, signal) =>
  new Promise((resolve) => {
    if (signal?.aborted) {
      resolve();
      return;
    }
    let timer = null;
    const done = () => {
      signal?.removeEventListener('abort', done);
      if (timer !== null) clearTimeout(timer);
      resolve();
    };
    timer = setTimeout(done, ms);
    signal?.addEventListener('abort', done, { once: true });
  });

/**
 * Run first-match lookups for `items` ([{ key, query }]) at a rate-limited
 * pace. Returns [{ key, query, match, error }]: `match` is the first
 * result or null; `error` is a message when that item's lookup threw
 * (errors never abort the run). Passing an AbortSignal cancels the
 * in-flight request too and returns what was fetched so far.
 *
 * onProgress fires twice per fresh item: before its request starts with
 * `current` set to the query in flight, and after the row lands with
 * `current: null` (so nothing is "in flight" during the pacing sleep).
 *
 * Identical queries are looked up once per run: repeats reuse the first
 * answer and skip both the request and its pacing slot (versioned names
 * often clean to the same term).
 */
export const runBulkLookup = async (items, { lookup = lookupITunes, intervalMs = BULK_LOOKUP_INTERVAL_MS, onProgress, signal } = {}) => {
  const rows = [];
  const perRunCache = new Map();
  for (let i = 0; i < items.length; i += 1) {
    if (signal?.aborted) break;
    const item = items[i];
    let outcome = perRunCache.get(item.query);
    const fresh = !outcome;
    if (fresh) {
      onProgress?.({ done: rows.length, total: items.length, current: item.query });
      let match = null;
      let error = null;
      try {
        const results = await lookup(item.query, { signal });
        match = results?.[0] ?? null;
      } catch (e) {
        // A cancelled in-flight request is not a failed row; stop cleanly.
        if (signal?.aborted) break;
        error = e?.message || 'Lookup failed';
      }
      outcome = { match, error };
      if (!error) perRunCache.set(item.query, outcome);
    }
    rows.push({ key: item.key, query: item.query, match: outcome.match, error: outcome.error });
    onProgress?.({ done: rows.length, total: items.length, current: null });
    if (fresh && i < items.length - 1 && !signal?.aborted) {
      await sleep(intervalMs, signal);
    }
  }
  return rows;
};
